...
def test_find_latest_queue(tmp_path: Path) -> None:
    (tmp_path / "2026_03_10_skill_candidates.json").write_text("{}")
    (tmp_path / "2026_03_17_skill_candidates.json").write_text("{}")
    result = find_latest_queue(tmp_path)
    assert result is not None
    assert "2026_03_17" in result.name


def test_find_latest_queue_mixed_date_separators(tmp_path: Path) -> None:
    (tmp_path / "2026_03_19_skill_candidates.json").write_text("{}")
    (tmp_path / "2026-03-25_skill_candidates.json").write_text("{}")
    result = find_latest_queue(tmp_path)
    assert result is not None
...
def test_load_queue(tmp_path: Path) -> None:
    ...
    queue_file = tmp_path / "2026_03_17_skill_candidates.json"
